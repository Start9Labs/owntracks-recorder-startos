...
import { sdk } from '../sdk'
import { mqttPort, recorderMqttUsername } from '../utils'
...
// Subtopics OwnTracks phones may have set with the retain flag. Publishing an
// empty payload with `-r` to each clears the broker's retained state so
// subscribers (other phones, the recorder, etc.) won't be re-served the stale
// data on their next connect.
...
// that has ever published a location. Runs in a temporary read-only recorder
// subcontainer so we don't perturb the running daemon.
...
// Defensive validation before passing user/device into the rm command — the
// values come from a dropdown populated by listRecordedPairs, but constraining
// them explicitly closes off any path-traversal surface.
...
export const forgetTracks = sdk.Action.withInput(
  'forget-tracks',

  {
    name: i18n('Forget Device Tracks'),
    description: i18n(
      "Wipe a device's location history and last-known position from the map, based on the user/device pair.",
    ),
    warning: null,
    allowedStatuses: 'any',
    group: null,
  ...
  async ({ effects, input }) => {
    ...
    // Clear the broker's retained-message cache for the device. Otherwise other
    // phones subscribed to owntracks/+/+ would be re-served the old location on
    // their next reconnect and the marker would reappear. The bundled recorder
    // account has readwrite on owntracks/#, so it can publish the tombstones.
    const recorderPassword =
      (await storeJson.read((s) => s.recorderPassword).once()) ?? ''
    await sdk.SubContainer.withTemp(
      effects,
      { imageId: 'mosquitto' },
      sdk.Mounts.of(),
